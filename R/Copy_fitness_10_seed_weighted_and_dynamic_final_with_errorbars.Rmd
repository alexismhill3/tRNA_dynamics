...
opt <- 0.6
nonopt <- 0.26
weighted_output_dir <- sprintf("data/simulation/revised_weighted_opt%.1f_nonopt%.2f", opt, nonopt)
...
for (seed in seeds) {
  seed_data <- NULL
    for (fop in c(0.1, 0.7)) {
      tmp <- load_sim(paste0(weighted_output_dir,"/weighted_opt",opt,"_nonopt",nonopt,"_",seed,"_fop",fop,".tsv"))
...
setwd("/stor/scratch/Wilke/amh7958/phage_simulation_kelly")

dir <- "data/simulation/phage/copy_revised_dynamic_pref0.7_charge0.5_ribospeed1.5/"
fops <- c(0.7, 0.1)

dynamic_all_times <- NULL
for (fop in fops) {
  tmp <- read_tsv(paste0(dir, "trna_phage_pref0.7_1", "_fop", fop, "_ribospeed1.5_trna50000.tsv"))
  tmp <- tmp %>% 
    mutate(time = round(time)) %>% 
    mutate(fop = fop)
  if (is.null(dynamic_all_times)) {
    dynamic_all_times <- tmp
  }
  else {
    dynamic_all_times <- rbind(dynamic_all_times, tmp)
  }
}
```
...
```{r}
color <- "#0090C1"
facet_labels <- c("0.1" = "Deoptimized", "0.7" = "WT")

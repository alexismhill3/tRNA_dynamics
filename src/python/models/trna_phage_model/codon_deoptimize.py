...
"""
    ...
    for Fop_target in [i / 10.0 for i in range(0, 11)]:
        if Fop_target < score:
            maximize = False
        else:
            maximize = True
...
if __name__ == "__main__":
    ...
    parser.add_argument('-x', '--exclude-front', default=14, type=int,
                        metavar='n',
                        help='number of codons to exclude from the front of the sequence')
    parser.add_argument('-X', '--exclude-back', default=14, type=int,
                        metavar='n',
                        help='number of codons to exclude from the back of the sequence')
    parser.add_argument('-a', '--Fop-start', default=0.5, type=float,
                        metavar='x',
                        help='starting value for Fop')
    parser.add_argument('-d', '--Fop-step', default=-0.1, type=float,
                        metavar='x',
                        help='step size from one Fop value to the next')
    parser.add_argument('-e', '--Fop-stop', default=0.1, type=float,
                        metavar='x',
                        help='end value for Fop')
